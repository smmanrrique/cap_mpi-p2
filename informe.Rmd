...
resumen_latency_exp = sqldf("SELECT   PACKET_SIZE, NPROC, MIN(COM_TIME) BEST_TIME, MAX(COM_TIME) WORST_TIME, AVG(COM_TIME) AVERAGE, ROUND((PACKET_SIZE/MIN(COM_TIME))/1048576,4) BANDWIDTH, ROUND((PACKET_SIZE/AVG(COM_TIME))/1048576, 4) THROUGHPUT  FROM latency_exp  GROUP BY NPROC,PACKET_SIZE ")

resumen_latency_exp

library(ggplot2)
ggplot(resumen_latency_exp, aes(x=PACKET_SIZE, y=AVERAGE,  group = NPROC )) + 
  geom_line(colour="red")  + 
  geom_point( size=2, shape=11, fill="white", colour="red") + 
  theme_minimal()

library(ggplot2)
ggplot(resumen_latency_exp, aes(x=PACKET_SIZE, y=AVERAGE, group = NPROC, colour =NPROC )) + 
  geom_line()  + 
...
sqldf("SELECT  COUNT(DISTINCT(NODE)) MACHINES, NPROC PROCESS, MIN(COM_TIME) WORST_TIME, MAX(COM_TIME) BEST_TIME, ROUND(AVG(COM_TIME),4) AVERAGE FROM bandwidth GROUP BY  NPROC")
...
bandwidth_exp = filter(latency_exp, TYPE==" RS" )

resumen_bandwidth_exp = sqldf("SELECT   PACKET_SIZE, NPROC, MIN(COM_TIME) BEST_TIME, MAX(COM_TIME) WORST_TIME, AVG(COM_TIME) AVERAGE, ROUND((PACKET_SIZE/MIN(COM_TIME))/1048576, 4) BANDWIDTH, ROUND((PACKET_SIZE/AVG(COM_TIME))/1048576, 4) THROUGHPUT  FROM bandwidth_exp  GROUP BY NPROC,PACKET_SIZE ")
...
#### Experimentos de Broadcast 
```{r}
# Read and import csv
broadcast_exp <- read.csv("C:/Users/smmanrrique/3D Objects/unizar/cap/cap_mpi-p2/results/broadcast_experiment_10.csv")
broadcast_exp = filter(latency_exp, TYPE==" RS" )

resumen_broadcast_exp = sqldf("SELECT   PACKET_SIZE, NPROC, MIN(COM_TIME) BEST_TIME, MAX(COM_TIME) WORST_TIME, AVG(COM_TIME) AVERAGE, ROUND((PACKET_SIZE/MIN(COM_TIME))/1048576, 4) BANDWIDTH, ROUND((PACKET_SIZE/AVG(COM_TIME))/1048576, 4) THROUGHPUT  FROM broadcast_exp  GROUP BY NPROC,PACKET_SIZE ")
